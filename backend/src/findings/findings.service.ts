...
export class FindingsService {
  ...
  async review(user: ReviewUser, sessionId: string, findingId: string, dto: ReviewFindingDto) {
    const session = await this.prisma.session.findUnique({ where: { id: sessionId } });
    if (!session) throw new NotFoundException('Session not found');
    ...
    if (!finding || finding.sessionId !== sessionId) throw new NotFoundException('Finding not found');
    if (finding.reviewerDecision) throw new ConflictException('Finding has already been reviewed');

    const action = dto.action as ReviewAction;
    const finalRecommendation =
      dto.action === ReviewActionEnum.edited ? dto.finalRecommendation! : finding.recommendation;
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      // Transition session to IN_REVIEW if not already
      if (session.status === 'ANALYSED') {
        await tx.session.update({ where: { id: sessionId }, data: { status: 'IN_REVIEW' } });
      }
    });
